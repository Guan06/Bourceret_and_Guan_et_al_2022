...
p1a <- pcoa(dmr, design, 12, "Genotype", "Stage", 2, kingdom) +
...
root$Genotype_Stage <- factor(root$Genotype_Stage,
                              levels = order_new,
                              ordered = TRUE)
p1b <- box(root, "Genotype", "Stage", "Genotype_Stage", "Shannon") +
...
write.table(sig_bac_root, "s5_sig_bac_root.txt", quote = F, sep = "\t", row.names = F)
...
p2a <- pcoa(dmr, design, 12, "Genotype", "Stage", 2, kingdom) +
        theme(legend.position = "none", axis.text = element_text(size = 18)) +
        scale_shape_manual(values = c(1, 16))
...
root$Genotype_Stage <- factor(root$Genotype_Stage,
                            levels = order_new,
                            ordered = TRUE)
p2b <- box(root, "Genotype", "Stage", "Genotype_Stage", "Shannon") +
    theme(legend.position = "none") + scale_x_discrete(limits = order_new) +
        scale_shape_manual(values = c(1, 16))
...
write.table(sig_fun_root, "s5_sig_fun_root.txt", quote = F, sep = "\t", row.names = F)
...
p3b <- box(root, "Genotype", "Stage", "Genotype_Stage", "Shannon") +
    theme(legend.position = "none") + scale_x_discrete(limits = order_new) +
    scale_shape_manual(values = c(1, 16))
...
write.table(sig_oom_root, "s5_sig_oom_root.txt", quote = F, sep = "\t", row.names = F)
...
ggsave("Figure_S5.pdf", all, width = 16, height = 10)
