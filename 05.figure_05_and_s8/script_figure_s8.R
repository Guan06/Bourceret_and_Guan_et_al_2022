...
s12 <- plot_grid(b, a, nrow = 2, ncol = 1, labels = 'auto',
                align = 'v', axis = 'l', rel_heights = c(1, 1.3))
ggsave("Figure_S12.pdf", s12, height = 6, width = 6)
...
write.table(sig_a, "Figure_S12b.txt", quote = F, sep = "\t", row.names = F)
...
sig_b$Sig_FDR <- ifelse(as.numeric(as.character(sig_b$FDR)) < 0.05,
                        TRUE, FALSE)
write.table(sig_b,"Figure_S12a.txt", quote = F, sep = "\t", row.names = F)
